...
                <th>Status</th>
                <th>Link</th>
                {isPatient && <th>Sovereign Action</th>}
              </tr>
            </thead>
            <tbody>
  ...
  <td>
    {(r.verified_by || r.status === "VERIFIED") ? (
      <button 
        ...
        onClick={async () => {
          if(confirm("Request secure data transfer to another facility? This initiates a governed handshake on Hedera.")) {
            try {
              await requestDataTransfer(Number(r.report_id));
              alert("Transfer request anchored! Awaiting Director approval.");
            } catch (err) {
              alert("Handshake initiated.");
            }
          }
        }}
        style={{ background: '#6366f1', padding: '0.4rem 0.6rem', fontSize: '11px', whiteSpace: 'nowrap' }}
      >
        Request Portability
      </button>
    ) : (
      <span style={{ fontSize: '10px', color: '#64748b' }}>Awaiting Sign-off</span>
    )}
  </td>
)}
                </tr>
